...
import { ArrowLeft, Camera, Check, RotateCcw, Loader2 } from "lucide-react";
import { api } from "../../../convex/_generated/api";
import { Button } from "@/components/ui/button";
import { LockMotif } from "@/components/deco-art";
import { env } from "@/env";
...
export default function UploadPage() {
  ...
  const generateUploadUrl = convexEnabled
    ? useMutation(api.photos.generateUploadUrl)
    : null;
  // eslint-disable-next-line react-hooks/rules-of-hooks
  ...
  return (
    <div className="flex min-h-screen flex-col">
      <header className="border-b border-bronze/30 bg-black/40 px-4 py-4 backdrop-blur-sm sm:px-6">
      ...
      <main className="mx-auto flex w-full max-w-md flex-1 flex-col items-center px-5 py-8">
        <LockMotif className="h-12 w-12" />
        ...
        {/* Viewfinder */}
        <div className="relative aspect-[3/4] w-full overflow-hidden rounded-md border-2 border-bronze/50 bg-noir shadow-gold">
          {/* Live video */}
          <video
            ref={videoRef}
            playsInline
          ...
          {stage === "error" && (
            <div className="absolute inset-0 flex items-center justify-center bg-noir p-6 text-center">
          ...
          <div className="pointer-events-none absolute left-0 top-0 size-6 border-l-2 border-t-2 border-gold" />
          <div className="pointer-events-none absolute right-0 top-0 size-6 border-r-2 border-t-2 border-gold" />
          <div className="pointer-events-none absolute bottom-0 left-0 size-6 border-b-2 border-l-2 border-gold" />
          <div className="pointer-events-none absolute bottom-0 right-0 size-6 border-b-2 border-r-2 border-gold" />
        </div>
        ...
        <input
          ...
          className="mt-6 w-full rounded-md border border-bronze/40 bg-card/40 px-4 py-2.5 text-center font-display text-sm uppercase tracking-[0.18em] text-gold placeholder:text-muted-foreground/60 focus:border-gold focus:outline-none focus:ring-[3px] focus:ring-ring/40"
        />
        ...
        {!convexEnabled && (
          <p className="mt-5 text-center text-xs leading-relaxed text-muted-foreground/70">
            Preview mode — run{" "}
            <code className="text-bronze">npx convex dev</code> to connect the
